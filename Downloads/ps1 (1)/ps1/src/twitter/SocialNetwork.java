	    ...
	    for (Tweet tweet : tweets) {
	        ...
	        for (String mentioned : mentions) {
	            if (!mentioned.equals(author)) {
	                followsGraph.computeIfAbsent(author, k -> new HashSet<>()).add(mentioned);
	            }
	        }
	    }
	...
	public static List<String> influencers(Map<String, Set<String>> followsGraph) {
	    ...
	    for (Map.Entry<String, Set<String>> entry : followsGraph.entrySet()) {
	        for (String followed : entry.getValue()) {
	            String name = followed.toLowerCase();
	            followerCount.put(name, followerCount.getOrDefault(name, 0) + 1);
	        }
	    }
